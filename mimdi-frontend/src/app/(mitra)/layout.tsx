...
export default function PartnerLayout({
  ...
  const navItems = [
    ...
    { href: '/partner/transactions', label: 'Transaksi', icon: CreditCard },
    { href: '/partner/affiliate', label: 'Afiliasi', icon: Share2 },
    { href: '/partner/clients', label: 'Klien Saya', icon: Users },
    { href: '/partner/commissions', label: 'Laporan', icon: DollarSign }, // <-- Tambahkan ini
  ];

  if (isLoading || !user) {
    return <div className="flex h-screen items-center justify-center">Memverifikasi akses mitra...</div>;
  }
            ...
            <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
              <Link href="/partner/dashboard" className="flex items-center gap-2 font-semibold">
                <img src="/logo.svg" alt="Logo" className="h-6 w-6" />
                 {!isCollapsed && <span className="transition-opacity duration-300">Mitra Panel</span>}
              </Link>
            </div>
